...
export default function IncidentIndexMapPage({ incidents }) {
  ...
  // Получение данных о качестве воздуха и ветре по координатам
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      incidents.forEach(incident => {
        ...
        // Создаем popup с HTML содержимым
        ...
        marker.getElement().addEventListener('click', (e) => {
          ...
          const popupContent = `
            <div style="font-family: system-ui, sans-serif; min-width: 200px;">
              <strong style="font-size: 14px;">${incident.anamnesis || 'Нет описания'}</strong>
              ${incident.address ? `<br/><small style="color: #666;">${incident.address}</small>` : ''}
              <div style="margin-top: 10px; padding-top: 8px; border-top: 1px solid #e0e0e0;">
                <button 
                  class="air-quality-btn" 
                  data-lat="${lat}" 
                  data-lon="${lon}"
          ...
          popup.setHTML(popupContent).addTo(map.current);
          
          // Добавляем обработчик для кнопки после открытия popup
          setTimeout(() => {
            const btn = document.querySelector('.air-quality-btn');
            if (btn) {
  ...
  // Функция для получения цвета AQI
